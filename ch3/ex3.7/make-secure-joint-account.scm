...
(define (make-joint acc acc-password joint-acc-password)
  (define (withdraw amount)
    ((acc acc-password 'withdraw) amount))
  (define (deposit amount)
    (acc acc-password 'deposit) amount)
  (define (dispatch attempted-password m)
    (cond ((not (eq? attempted-password joint-acc-password)) (error "Incorrect password --MAKE-JOINT-ACCOUNT"))
	  ((eq? m 'withdraw) withdraw)
	  ((eq? m 'deposit) deposit)
	  (else (error "Unknown request: MAKE-JOINT-ACCOUNT" m))))
  dispatch)
