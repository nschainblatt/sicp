(define (main)
  ...
  (let* ((int1 (make-integer 5))
    ...
    (println (=zero? polynomial4))   ;#t
    (println (=zero? polynomial2))   ;#f
    (println (=zero? polynomial3)))) ;#t
...
(define (install-raise-package)
  ...
  (define (complex->polynomial n)
    (make-polynomial 'x '())) ;; NOTE: returning 0 to make the mydrop always not pass the equality check, keeping it as a polynomial
...
(define (install-project-package)
  (define (polynomial->complex n)
    (make-complex-from-real-imag 0 0)) ;; NOTE: returning 0 to make the mydrop always not pass the equality check, keeping it as a polynomial
