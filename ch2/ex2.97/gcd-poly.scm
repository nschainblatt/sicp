  ...
  (define (apply-gcd-to-coefficients term-list)
    (if (empty-termlist? term-list)
      ...
      (let ((t1 (first-term term-list))
            (d (gcd-of-coefficients term-list))
            (rest (rest-terms term-list)))
        (adjoin-term (make-term (order t1) (mydiv (coeff t1) d)) (apply-gcd-to-coefficients rest)))))

  (define (gcd-of-coefficients terms)
    (define (get-list-of-coefficients term-list)
      (if (empty-termlist? term-list)
        '()
        (cons (coeff (first-term term-list)) (get-list-of-coefficients (rest-terms term-list)))))
    (let ((coefficients (get-list-of-coefficients terms)))
      (accumulate mygcd (car coefficients) (cdr coefficients))))
    ...
    (if (empty-termlist? L1)
      ...
      (let ((t1 (first-term L1))
        ...
        (if (gt (order t2) (order t1))
          ...
          (let* ((new-c (mydiv (coeff t1) (coeff t2)))
...
(define (install-integer-package)
  ...
  (put 'negate '(integer) negate-integer)
...
(define (install-rational-package)
  ...
  (define (cos-rat x)
    (make-rat (numerator (exact (cos (mydiv (numer x) (denom x)))))
              (denominator (exact (cos (mydiv (numer x) (denom x)))))))
  ...
  (put 'sqrt '(rational) (lambda (x) (tag (make-rat (mysqrt (numer x)) (mysqrt (denom x))))))
  (put 'atan '(rational rational) (lambda (x y) (tag (make-rat (numerator (exact (myatan (mydiv (numer x) (denom x)) (mydiv (numer y) (denom y)))))
                                                               (denominator (exact (myatan (mydiv (numer x) (denom x)) (mydiv (numer y) (denom y)))))))))
  (put 'sin '(rational) (lambda (x) (tag (make-rat (numerator (exact (mysin (mydiv (numer x) (denom x))))) (denominator (exact (mysin (mydiv (numer x) (denom x)))))))))
...
(define (install-raise-package)
  ...
  (define (rational->real n)
    (make-real (mydiv (apply-specific 'numer 'rational n) (apply-specific 'denom 'rational n))))
...
(define (install-project-package)
  ...
  (define (rational->integer n)
    (make-integer (round (mydiv (apply-specific 'numer 'rational n) (apply-specific 'denom 'rational n)))))
