;; 84 bits are required for the variable length encoding.
;; The smallest number of bits required if we used fixed length code would be 108, 3 bits per symbol for a total of 36 symbols in the sample message.

(define (main)
  (let ((pairs '((A 2) (NA 16) (BOOM 1) (SHA 3) (GET 2) (YIP 9) (JOB 2) (WAH 1)))
        (sample-message '(GET A JOB SHA NA NA NA NA NA NA NA NA GET A JOB SHA NA NA NA NA NA NA NA NA WAH YIP YIP YIP YIP YIP YIP YIP YIP YIP SHA BOOM)))
    (println (length (encode sample-message (generate-huffman-tree pairs))))))
