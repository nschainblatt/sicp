;; First implementation of rectangles. This only uses two segments for the length and height, because given one length and height, we know the
;; other sides as rectangles have two pairs of equal sides.
;; This however leaves room for error, as the two segments must have a vertex, otherwise the rectangle will be invalid.
;; The second implementation I will use three parameters to the constructor, a vertex point, a length, and a height.
