;; Specific =zero? implementations can be found in their respective packages, the single generic to utilize is found below that applies
;; the procedure based on the type.
...
(define (main)
  ...
  (let* ((rat1 (make-rational 5 10))
         (rat2 (make-rational 0 1))
         (complex1 (make-complex-from-real-imag 1 2))
         (complex2 (make-complex-from-real-imag 0 0))
         (num1 (make-scheme-number 3))
         (num2 (make-scheme-number 0)))
    (println (=zero? rat1))
    (println (=zero? rat2))
    (println (=zero? complex1))
    (println (=zero? complex2))
    (println (=zero? num1))
    (println (=zero? num2))))
