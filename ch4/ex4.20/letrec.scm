;; The reason Louis's reasoning is loose is that with a regular let is because you would not be able to do the 'fact' example or the 'even?/odd?' example in this exercise.
;; Both of those examples require the bindings to be created in a shared environment since they are recursively defined.
;; A regular let does not create the bindings in a shared environment, so when you invoke either even? or odd? in the let body, an error will occur because those lambdas
;; were defined in an environment not aware of the other binding dependency.
...
;; get the assignments, set the variables to unassigned
;; create setters for the assignments, setting them to their values
;; create a new body with the setters first, followed by the let body
;; create a new let with the assignments and body
(define (letrec->let exp)
  (let* ((assignments (let-assignments exp))
	 (body (let-body exp))
	 (new-assignments (map (lambda (a) (make-let-assignment (let-variable a) 'unassigned)) assignments))
	 (setters (map (lambda (a) (make-assignment (let-variable a) (let-value a))) assignments)))
    (make-let new-assignments (append setters body))))
...
(define (make-let-assignment var value)
  (list var value))
