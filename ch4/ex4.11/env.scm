...
(define (lookup-variable-value var env)
  (define (env-loop env)
    (define (scan bindings)
      (cond ((frame-bindings-empty? bindings)
	     (env-loop (enclosing-environment env)))
	    ((eq? var (binding-var (frame-first-binding bindings))) (binding-val (frame-first-binding bindings)))
	    (else (scan (frame-rest-bindings bindings)))))
...
(define (set-variable-value! var val env)
  (define (env-loop env)
    (define (scan bindings)
      (cond ((frame-bindings-empty? bindings)
	     (env-loop (enclosing-environment env)))
	    ((eq? var (binding-var (frame-first-binding bindings))) (set-binding-val! (frame-first-binding bindings) val))
	    (else (scan (frame-rest-bindings bindings)))))
    (if (eq? env the-empty-environment)
