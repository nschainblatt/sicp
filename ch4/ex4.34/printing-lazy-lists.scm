;; I tagged my cons procedure in order to handle it later when printing.
;; When printing I only print the car of the lazy-list to handle the case of infinite lists.
;; I print in the same format that printing streams do:
;; (cons 1 2)  => {1 ...}
;; (car (cons 1 (cons 2 '()))) => {2 ...}
...
(define (main)
  ...
  (eval '(define (cons x y)
	   (standard-list 'lazy-pair (lambda (m) (m x y)))) the-global-environment)
  (eval '(define (car C)
	   ((standard-car (standard-cdr C)) (lambda (x y) x))) the-global-environment)
  (eval '(define (cdr C)
	   ((standard-car (standard-cdr C)) (lambda (x y) y))) the-global-environment)
  (driver-loop))
...
(define (install-eval-package)
  ...
  (put 'eval 'call (lambda (exp env) (apply (actual-value (operator exp) env)
					    (operands exp)
					    env)))
...
(define (make-quote val)
  (list 'quote val))
(define (quoted? exp) (tagged-list? exp 'quote))
(define (text-of-quotation exp env)
  (let ((contents (cadr exp)))
    ;; If the contents of the quotation is a list we need to convert this into our lazy list.
    (if (pair? contents)
      (eval (quoted-list->lazy-list contents) env)
      contents)))

;; Since the lazy list procedures are apart of the language, we can create a derived expression that when evaluated will
;; construct our lazy list.
(define (quoted-list->lazy-list seq)
  (if (null? seq)
    '()
    (make-cons (make-quote (car seq)) (quoted-list->lazy-list (cdr seq)))))
...
;; Prints the car of the lazy-pair since the pair may be infinite.
(define (print-lazy-pair lazy-pair)
  (let* ((procedure (cadr lazy-pair))
	 (env (procedure-environment procedure))
	 (parameters (procedure-parameters procedure)))
    (display "{")
    (display (actual-value 'x env))
    (display " ")
    (display "...}")))
...
(define primitive-procedures
  (list (list 'car car)
	...
	(list 'standard-cons cons)
	(list 'standard-car car)
	(list 'standard-cdr cdr)
	(list 'null? null?)
...
(define (user-print object)
  (cond ((compound-procedure? object)
	 (display (list 'compound-procedure
			...
			'<procedure-env>)))
	((lazy-pair? object) (print-lazy-pair object))
	(else (display object))))
