...
;; This was to match up the traced process exactly with the logarithmic version.
...
(define (fast-expt-mod a n)
  (remainder (expmod-linear a n n) n))

(define (expmod-linear base exp m)
  (cond ((= exp 0) 1)
	((even? exp)
	  (remainder (* (expmod-linear base (/ exp 2) m) (expmod-linear base (/ exp 2) m)) m))
	(else 
	  (remainder (* base (expmod-linear base (- exp 1) m)) m))))
